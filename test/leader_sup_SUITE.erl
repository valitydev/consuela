...
start_slave(Name, CodePath) ->
    ...
    _Apps =
        [_ | _] = rpc:call(Node, genlib_app, start_application_with, [
            consuela,
            [
                {registry, #{
                    nodename => "consul0",
...
leader_uniqueness_holds(C) ->
    ...
    % pid() > atom()
    [Pid | Rest] = lists:reverse(lists:sort(Results)),
    _ = ?assert(is_pid(Pid)),
...
wait_leader(Name) ->
    ...
    after 3000 -> erlang:error({no_leader_found, Name})
    end.

start_leaders(Nodes, Args) ->
    {OkPids, []} = rpc:multicall(Nodes, supervisor, start_child, [
        kernel_safe_sup,
        #{
            id => leader,
